    ...
    /*
      {
        ...
        // Initialize the cif
        ffi_status ret = ffi_prep_cif (&cif, FFI_DEFAULT_ABI, 1, &ffi_type_sint,
      args); if (FFI_OK == ret) { clock_t b = clock(); for (i = 0; i < 10000000;
      ++i) { ffi_call (&cif, (void *) incr, &I, values);
                    }
                    clock_t e = clock();
...
static int test_lac_cif_variant()
{
    ...
    lac_variant v =
        (lac_variant){.type = &ffi_type_double, .value._double = 1.23};
