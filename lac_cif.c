...
{
    return ffi_prep_cif(&pcif->cif, FFI_DEFAULT_ABI, pcif->cif.nargs,
                        pcif->cif.rtype, pcif->cif.arg_types);
}
                          ...
                          ffi_type **arg_types)
{
    ...
    ffi_status ret =
        ffi_prep_cif_var(&p_->cif, FFI_DEFAULT_ABI, nfix, nfix + nargs,
                         p_->cif.rtype, p_->cif.arg_types);
    ensure(FFI_OK == ret);
