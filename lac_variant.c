...
#define FFI_POINTER                                                            \
    {                                                                          \
        sizeof(void *), offsetof(struct struct_align_pointer, x),              \
...
static ffi_type ffi_type_variant_union = {.size = 0,
                                          .alignment = 0,
                                          .type = FFI_TYPE_STRUCT,
                                          .elements =
                                              ffi_variant_union_elements};

#define X(A, B, C, D) &ffi_type_##B,
ffi_type *ffi_type_variant_union_elements[] = {FFI_TYPE_TABLE(X) NULL};
...
static void prep_union_type(ffi_type *union_type, ffi_type **union_elements)
{
    while (*union_elements) {
        ffi_cif cif;
        if (FFI_OK ==
            ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 0, *union_elements, NULL)) {
...
static ffi_type *ffi_variant_elements[] = {&ffi_type_variant_union,
                                           &ffi_type_pointer, NULL};

ffi_type ffi_type_variant = {.size = sizeof(lac_variant),
                             .alignment =
