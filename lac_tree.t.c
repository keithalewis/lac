...
void action(const void* p, int value, int level)
{
...
{
	{
		...
		/*void* v =*/ lac_tree_put(&t, DATUM("foo"), DATUM("val"));
		lac_tree_datum* val = lac_tree_get(&t, DATUM("foo"));
		ensure (val->size == 3);
	...
	}
	{
		...
		lac_tree_datum* val2 = lac_tree_get(&t, DATUM("foo"));
		ensure (val2 == 0);
	...
	}
	{
		...
		/*void* v =*/ lac_tree_put(&t, DATUM("foo"), DATUM("val2"));
		twalk(t, action);
		printf("\n");
		lac_tree_datum* val2 = lac_tree_get(&t, DATUM("foo"));
		val2 = val2;
